# -*- coding: utf-8 -*-
"""
# ## Feature_prognosis_bearing.py

Created on Thu Dec  9 08:44:34 2021
...
plt.rc('font', size=13)
rawData = 'C:/FEMTO/Dataset/Bearing1_1.csv'
os.chdir('C:/FEMTO/Dataset/Bearing1_1')
...
rr = pd.read_csv(rawData)
nLen = len(rr) 
blocks = nLen / 2560


ix_chunck = 0
for df in pd.read_csv(rawData, chunksize=25600):
    x = df['acc_vert'].values 
    ...
    features[ix_chunck, :] = np.concatenate([tmp1, tmp2])
    ix_chunck = ix_chunck + 1
    if (ix_chunck == 99 ):
        break
...
#    x = io.loadmat(file_list[ix_file])['x']
    ...
    df = read_csv(file_list[ix_file] , header=0, nrows=20480)           # Read raw signal file
    x = df['acc_vert'].values                            # Set the Column name
...
# 2. Feature selection for cabstone_prognosis
cycle = np.arange(0,N_file)
w = np.mat([0.33, 0.33, 0.33])

# Normalizing
m = np.mean(features, axis=0)
s = np.std(features, ddof=1, axis=0)
...
plt.ylim(0,0.5)
plt.bar(range(0, 15), cri)
plt.title('Feature selection critera')
